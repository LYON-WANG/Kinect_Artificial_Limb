  ...
  if(start_flag == 1)
  {
    ...
    if (userList.size()>0)
    {
      ...
      if (kinect.isTrackingSkeleton(userID))
      {
        ...
        Angle_Shoulder_Out     = int(shoulderAngle_XY);
        Angle_Shoulder_Forward = int((rightShoulder.z - rightElbow.z));
        Angle_Elbow            = int(elbowAngle_XY);
        
        /*------------Servo angle-----------*/
        a = int(map(Angle_Shoulder_Forward,-150,magnitude1,70,5));
        if(a>70) a=70; if(a<5) a=5;
        
        b = int(map(Angle_Shoulder_Out,20,55,140,180)); 
        if(b > 180) b = 180; if(b < 140) b = 140;
        
        c = int(map(Angle_Elbow,100,180,30,75)); 
